...
tac OUTCAR | grep -m1 -B$offst1 "magnetization (x" | tac >  temp.txt   # to create a file with required magnetization of last iteration
...
#        	echo -n -e "\t"$m>>POSCAR.mcif
                echo -n -e "\t" >> POSCAR.mcif
                printf '%2.3f' $m >> POSCAR.mcif
		done
        echo -n -e "\t |m| : $(tail -1 POSCAR.mcif| awk '{print sqrt($2^2+$3^2+$4^2)}')"
        echo -n -e "\t Theta in x-z Plane: $(tail -1 POSCAR.mcif| awk '{print atan2 ($4,$2) * 180 / 3.14159265359}')"
